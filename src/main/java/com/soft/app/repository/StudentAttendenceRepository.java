...
public interface StudentAttendenceRepository extends JpaRepository<StudentAttendence, Long>{
     @Query("select st from Student st where st not in(select ss from Student ss, classes cc, StudentAttendence sc where ss.id=sc.student.id and cc.id=sc.classes4.id and cc.id=?1 and sc.date= ?2)")
