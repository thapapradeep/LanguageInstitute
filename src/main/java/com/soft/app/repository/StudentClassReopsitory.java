...
public interface StudentClassReopsitory extends JpaRepository<StudentClass, Long>{
	@Query("Select stu from Student stu where stu NOT IN(Select s from Student s, classes c, Batch b, StudentClass st  where b.id=s.batch.id and s.id=st.student.id and c.id=st.classes.id and c.id=?1 and c.batch.id=s.batch.id)")
